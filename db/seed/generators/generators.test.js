...
describe('syllabusesGenerator outputs data in the proper format', () => {
  test('syllabusesData should be an object with the correct properties', () => {
    ...
    expect(typeof lessons[0].hoursToCompleteLesson).toBe('number');
    expect(typeof lessons[0].title).toBe('string');
    expect(typeof lessons[0].description).toBe('string');
    expect(Array.isArray(lessons[0].videos)).toBe(true);
    expect(Array.isArray(lessons[0].readings)).toBe(true);
